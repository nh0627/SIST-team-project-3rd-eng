				...
				<ul class="nav navbar-nav">
					...
					<i class="menu-icon fa fa-bar-chart-o"></i>View Score
						...
						<ul class="sub-menu children dropdown-menu show">
                            ...
                            <li class="active"><i class="fa fa-folder"></i><a href="${pageContext.request.contextPath}/manager/score/bystudent">Score by Student</a></li>
                        </ul>
					</li>
				...
				<div class="page-header float-right">
					<div class="page-title">
						<ol class="breadcrumb text-right">
							<li>View Score</li>
							<li><a href="${pageContext.request.contextPath}/manager/score/bystudent">Score by Student</a></li>
							<li class="active">Course History</li>
						</ol>
					</div>
				</div>
			</div>
			...
			<div class="animated fadeIn">
				<div class="row">
					...
					<div class="col-lg-12">
										...
										<div class="col-sm-12">
											<table id="bootstrap-data-table" class="table table-striped table-bordered dataTable no-footer" role="grid" aria-describedby="bootstrap-data-table_info">
												...
												<tbody>
													...
													<c:forEach var="s" items="${ocList}">
														<tr>
															<td>${s.oc_id}</td>
															<td>${s.oc_title}</td>
															<td><df:dateForm date="${s.oc_start_date}" />~<br><df:dateForm date="${s.oc_end_date}" /></td>
															<td>${s.classroom_name}</td>
															<td>${s.learning_status}</td>
															<td><df:dateForm date="${s.course_end_date}" /></td>
															<td><button type="button" class="btn btn-info btn-sm btn-ss-check" value="${s.oc_id}"><i class="fa fa-search"></i></button></td>
														</tr>
													...
													<c:if test="${count==0}">
														<tr>
															<td colspan='12'>There is no course history.</td>
														</tr>
													</c:if>
						...
						<!-- .content -->
						<form id ="subject-list-w-student" action="${pageContext.request.contextPath}/manager/score/bystudent/courselist/scorelist" method="POST">

							<input type="hidden" name="student_id" id="student_id" value="${student_id}"/>
							<input type="hidden" name="oc_id" id="oc_id" />
							<input type="hidden" name="oc_title" id="oc_title" />
							<input type="hidden" name="oc_date" id="oc_date" />
					...
					$(document).ready(function(){
						...
						$("button.btn-ss-check").click(function(){
							...
							var oc_id = $(this).val();
							var oc_title = $(this).parent().siblings().eq(1).text();
							var oc_date = $(this).parent().siblings().eq(2).text();

							$("input#oc_id").val(oc_id);
							$("input#oc_title").val(oc_title);
							$("input#oc_date").val(oc_date);

		        	    	$("form#subject-list-w-student").submit();
