...
<script>
	$(document).ready(function() {
		$("button.btnCancleCourse").on("click", function() {
			if (confirm("Would you like to delete the selected data?")) {
				var student_id = "${studentInfo.student_id}";
				var oc_id = $(this).val();
				location.assign("${pageContext.request.contextPath}/manager/student/doCoursecancel?student_id=" + student_id + "&oc_id=" + oc_id );
			}
		});
	});
</script>
				...
				<div class="page-header float-right">
					<div class="page-title">
						<ol class="breadcrumb text-right">
							...
							<li class="active">Course Cancellation</li>
						</ol>
					</div>
				</div>
			</div>
					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-header">
								<strong class="card-title">Available Course List to Cancel for ${studentInfo.student_id}(${studentInfo.name_}/${studentInfo.ssn})</strong>
							</div>
							<div class="card-body">
								<table id="" class="table table-striped table-bordered">
									...
									<tbody>
									<c:if test="${cancelListCount>0}">
									<c:forEach var="c" items="${cancelList}">
										<tr>
											<td>${c.oc_id}</td>
											<td>${c.oc_title}</td>
											<td>
												<c:set var="oc_start_date" value="${c.oc_start_date}" />
												<c:if test="${oc_start_date!=null}">
													<df:dateForm date="${oc_start_date}" />
												</c:if>
												<br>~
												<c:set var="oc_end_date" value="${c.oc_end_date}" />
											...
											<td>
												<c:set var="course_end_date" value="${c.course_end_date}" />
												<c:if test="${course_end_date!=null}">
													<df:dateForm date="${course_end_date}" />
												</c:if>
											</td>
											<td><a href="#"><button type="button"
														class="btn btn-info btn-sm btnCancleCourse" value="${c.oc_id}">Cancel</button></a></td>
										</tr>
									</c:forEach>
									</c:if>
									<c:if test="${cancelListCount==0}">
										<tr>
											<td colspan="7">There is no course to cancel.</td>
										</tr>
									</c:if>
									</tbody>
								</table>
