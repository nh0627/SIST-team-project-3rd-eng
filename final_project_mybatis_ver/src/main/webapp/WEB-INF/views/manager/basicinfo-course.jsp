	...
	$(document).ready(function() {
		$("button.btnDeleteCourse").on("click", function() {
			if (confirm("Would you like to delete the selected data?")) {
				var bc_id = $(this).val();
				location.assign("${pageContext.request.contextPath}/manager/basicinfo/course/delete?bc_id=" + bc_id);
			}
		});
	});
</script>
	...
	<div id="right-panel" class="right-panel">
				...
				<div class="page-header float-right">
					<div class="page-title">
						<ol class="breadcrumb text-right">
							...
							<li class="active">Basic Course</li>
						</ol>
					</div>
				</div>
			</div>
					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-header">
								<strong class="card-title">Basic Course List</strong>
								<button type="button"
									class="btn btn-info btn-sm btn-margin-left" data-toggle="modal"
									data-target="#addCourse">Add New</button>
							</div>
							<div class="card-body">
								...
								<table id="bootstrap-data-table"
									...
									<tbody>
										<c:forEach var="bc" items="${bcList_ds}">
											<tr>
												<td>${bc.bc_id}</td>
												<td>${bc.bc_title}</td>
												<td><button type="button" class="btn btn-info btn-sm btnDeleteCourse" value="${bc.bc_id}" ${(bc.delete_status=='Y')?"":"disabled"}>Delete</button></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
		...
		<div class="modal fade" id="addCourse" role="dialog">
			<div class="modal-dialog">
				...
				<form id="new insert" action="${pageContext.request.contextPath}/manager/basicinfo/course/insert" method="POST" >
					<div class="modal-header">
						<h4 class="modal-title">Add New Basic Course</h4>
						<button type="button" class="close" data-dismiss="modal">&times;</button>
					</div>
					<div class="modal-body">

						<div class="form-group">
							<label for="bc_title" class=" form-control-label">Course Title</label> <input
								id="bc_title" name="bc_title" placeholder="Enter a course title" class="form-control" required>
						</div>
