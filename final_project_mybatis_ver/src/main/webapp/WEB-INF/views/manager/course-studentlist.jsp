...
<script>
	$(document).ready(function() {
        ...
        $("button.btnExcel").on("click", function() {
			if (confirm("Would you like to save the data in an Excel format?")) {
				location.assign("${pageContext.request.contextPath}/manager/course/studentlist/excel?oc_id=${oc_id}")
			}
        });
					...
					<div class="col-lg-12">
						<div class="card">
							<div class="card-header">
								<strong class="card-title">${ocInfo.oc_title}(<c:set var="oc_start_date" value="${ocInfo.oc_start_date}"/><c:if test="${oc_start_date!=null}"><df:dateForm date="${ocInfo.oc_start_date}"/></c:if> ~ <c:set var="oc_end_date" value="${ocInfo.oc_end_date}"/><c:if test="${oc_end_date!=null}"><df:dateForm date="${ocInfo.oc_end_date}"/></c:if>) Student List</strong>
							...
							<div class="card-body">
								<table class="table table-striped table-bordered">
									...
									<tbody>
										<c:forEach var="l" items="${oc_studentSearch}">
										<tr>
											<td>${l.student_id}</td>
											<td>${l.name_}</td>
											<td>${l.ssn}</td>
											<td>${l.phone}</td>
											<td><df:dateForm date="${l.initial_reg_date}"/></td>
