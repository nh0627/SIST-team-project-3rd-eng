...
$(document).ready(function() {
	// 중도탈락 처리를 위해 modal창 hidden태그에 수강생id와 개설과정id 전달
	$("button.btnDropout").on("click", function(){
		var student_id = "${studentInfo.student_id}";
		var oc_id= $(this).val();
		$("div#student-dropout input#student_id").val(student_id);
		$("div#student-dropout input#oc_id").val(oc_id);
	});
});
...
<body>
	...
	<div id="right-panel" class="right-panel">
				...
				<div class="page-header float-right">
					<div class="page-title">
						<ol class="breadcrumb text-right">
							<li><a
								href="${pageContext.request.contextPath}/manager/student">Manage Student
									</a></li>
							<li class="active">Drop Out</li>
						</ol>
					</div>
				</div>
			</div>
					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-header">
								<strong class="card-title">Available Course List to Drop out for ${studentInfo.student_id}(${studentInfo.name_}/${studentInfo.ssn})
									</strong>
							</div>
							<div class="card-body">
								<table id="" class="table table-striped table-bordered">
									...
									<tbody>
							 		<c:if test="${dropoutListCount>0}">
									<c:forEach var="d" items="${dropoutList}">
										<tr>
											<td>${d.oc_id}</td>
											<td>${d.oc_title}</td>
											<td>
												<c:set var="oc_start_date" value="${d.oc_start_date}" />
												<c:if test="${oc_start_date!=null}">
													<df:dateForm date="${oc_start_date}" />
												</c:if>
												<br>~
												<c:set var="oc_end_date" value="${d.oc_end_date}" />
												<c:if test="${oc_end_date!=null}">
													<df:dateForm date="${oc_end_date}" />
												</c:if>
											</td>
											<td>${d.classroom_name}</td>
											<td>${d.learning_status}</td>
											<td>
												<c:set var="course_end_date" value="${d.course_end_date}" />
												<c:if test="${course_end_date!=null}">
													<df:dateForm date="${course_end_date}" />
												</c:if>
											</td>
											<td><a href="#"><button type="button"
														class="btn btn-info btn-sm btnDropout" data-toggle="modal"
														data-target="#student-dropout" value="${d.oc_id}">Drop out</button></a></td>
										</tr> 
									</c:forEach>
								 	</c:if>
								 	<c:if test="${dropoutListCount==0}">
										<tr>
											<td colspan="7">There is no course to drop out.</td>
										</tr>
									</c:if>
									</tbody>
								</table>
		...
		<!-- Modal -->
		<div class="modal fade" id="student-dropout" role="dialog">
			<div class="modal-dialog">

				<!-- Modal content-->
				<div class="modal-content">
					<div class="modal-header">
						<h4 class="modal-title">Drop out</h4>
						<button type="button" class="close" data-dismiss="modal">&times;</button>
					</div>
					<form id="doDropout" action="${pageContext.request.contextPath}/manager/student/doDropout" method="POST" >
					<div class="modal-body">
						<div class="form-group">
							<label for="testdate">Drop date</label> <input type="date"
								class="form-control" id="dropout_date" name="dropout_date" required>
						</div>
						<div class="form-group">
